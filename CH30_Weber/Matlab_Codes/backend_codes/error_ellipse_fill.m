function [h,Xout,Yout]=error_ellipse_fill(varargin)
...
%   h1=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style,'linewidth',3);hold on
  h1=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'linewidth',1.5);hold on
  [y,z,x] = getpoints(Cyz,prop.clip);
%   h2=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style,'linewidth',3);hold on
  h2=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'linewidth',1.5);hold on
  [z,x,y] = getpoints(Czx,prop.clip);
%   h3=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style,'linewidth',3);hold on
  h3=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,'linewidth',1.5);hold on
...
%   h1=plot(scale*(x0+k*x),scale*(y0+k*y),'color',prop.style,'linewidth',3);
%   h1=plot(scale*(x0+k*x),scale*(y0+k*y),prop.style,'linewidth',3);

Xout = scale*(x0+k*x);
Yout = scale*(y0+k*y);

%  fill(scale*(x0+k*x),scale*(y0+k*y),prop.style); hold on;
  h1=plot(scale*(x0+k*x),scale*(y0+k*y),'color',prop.style,'linewidth',1.5);
  set(h1,'zdata',z+1)
  if nargout
    h=h1;
  end
